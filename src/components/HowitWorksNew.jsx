...
const howitworkssection = howitworks.map(item =>
    <div className='w-[90%] md:w-[380px] h-[400px] gap-4 bg-white shadow-md rounded-2xl border-2 border-blue-100 flex flex-col items-center p-4'>
        <h3 className='text-[22px] font-bold'>Step {item.step}</h3>
        <div>
            {item.icon}
        </div>
        ...
        <button className='w-[180px] h-[50px] mt-2 bg-gradient-to-r from-[#5c07B7] via-blue-500 text-white to-[#5c07B7] rounded-xl text-[18px] font-bold shadow-md'>Join waitlist</button>
    </div>
)

const HowitWorksNew = () => {
  return (
    <main className='w-full mt-24 py-24'>
         <motion.h1
        ...
        <div className='w-[90%] mx-auto mt-24 flex flex-wrap justify-center gap-4'>
            {howitworkssection}
        </div>
    </main>
  )
